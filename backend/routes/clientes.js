...
router.get('/buscar/inteligente', async (req, res, next) => {
  try {
    ...
    const result = await db.query(`
        ...
        WHERE
          ...
          OR (
            @cedula IS NULL AND (
              ...
              OR EXISTS (
                SELECT 1
                FROM dbo.cat_Provincia pr
                WHERE pr.N_provincia = c.C_provincia
                  AND pr.D_descripcion COLLATE Latin1_General_CI_AI LIKE @startsWith
              )
              OR EXISTS (
                SELECT 1
                FROM dbo.cat_Canton ca
                WHERE ca.N_canton = c.C_canton
                  AND ca.C_provincia = c.C_provincia
                  AND ca.D_descripcion COLLATE Latin1_General_CI_AI LIKE @startsWith
              )
              OR EXISTS (
                SELECT 1
                FROM dbo.cat_Distrito di
                WHERE di.N_distrito = c.C_distrito
                  AND di.C_canton = c.C_canton
                  AND di.D_descripcion COLLATE Latin1_General_CI_AI LIKE @startsWith
              )
            )
          )
        ORDER BY
      ...
      JOIN dbo.Cliente c ON c.C_cliente = cand.C_cliente
