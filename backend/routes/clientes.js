/**
 * routes/clientes.js
 * Adaptado al esquema real de Query_proyecto.sql:
 * dbo.Cliente(C_cliente, D_numero_identificacion, D_nombre_1, ...)
 */
...
router.get('/buscar/inteligente', async (req, res, next) => {
  try {
    ...
    const result = await db.query(`
      EXEC dbo.sp_BusquedaInteligenteCliente
        @D_termino = @D_termino,
        @D_cedula = @D_cedula,
        ...
        @Q_limite = @Q_limite;
    `, [
      { name: 'D_termino', value: termino || null },
      { name: 'D_cedula', value: cedula || null },
      { name: 'C_provincia', value: provincia ? Number(provincia) : null },
      { name: 'C_canton', value: canton ? Number(canton) : null },
      { name: 'C_distrito', value: distrito ? Number(distrito) : null },
      { name: 'Q_limite', value: Number(limite) || 50 }
    ]);
